...
// Build:  ./build.sh trailer   (also writes ./resources/trailer/architecture)
// Run:    ./resources/trailer/architecture
// Stills: ./resources/trailer/architecture --shot
// Record: ./resources/trailer/architecture --record resources/trailer/architecture.mp4
//
...
static void text(Font f, const char *s, float x, float y, float sz, Color c) {
    tfont_draw(f, s, x, y, sz, 0.4f, c);
}
static float tw(Font f, const char *s, float sz) {
    return tfont_width(f, s, sz, 0.4f);
}
...
static void usage(void) {
    fprintf(stderr,
        "usage: ./resources/trailer/architecture [flags]\n"
        "  --shot [FILE]     export a still (default resources/trailer/shots/loop.png)\n"
        "  --record [FILE]   record one %gs loop (default resources/trailer/architecture.mp4)\n"
...
int main(int argc, char **argv) {
    ...
    const char *record_path = "resources/trailer/architecture.mp4";
    float seek = -1.0f;

    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--shot") == 0) {
            shot = "resources/trailer/shots/loop.png";
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_S)) {
            ensure_shots_dir();
            draw_frame(&v, L);
            export_png("resources/trailer/shots/loop.png");
        }
