...
static void text_c(Font font, const char *s, float x, float y, float size, Color c) {
    Font f = tfont_fit(font, (int)(size + 0.5f));
    Vector2 sz = MeasureTextEx(f, s, (float)f.baseSize, 0);
    DrawTextEx(f, s, (Vector2){roundf(x - sz.x * 0.5f), roundf(y)}, (float)f.baseSize, 0, c);
}
...
void plot_scale_draw_box(Font title, Font body, Shader *star, float reveal, float alpha,
    ...
    Font flab = tfont_fit(body, (int)(lab + 0.5f));
    float sps_w = MeasureTextEx(flab, eq_sps, (float)flab.baseSize, 0).x;
    float vram_w = MeasureTextEx(flab, eq_vram, (float)flab.baseSize, 0).x;
    ...
    DrawTextEx(flab, eq_sps, (Vector2){roundf(legend_x + 48), roundf(legend_y)},
        (float)flab.baseSize, 0, ca(PUFF_CYAN, alpha));
    float g0 = legend_x + 48.0f + sps_w + 40.0f;
    DrawLineEx((Vector2){g0, legend_y + 10},
        (Vector2){g0 + 36, legend_y + 10}, 3.5f, ca(PUFF_GREEN, alpha));
    DrawTextEx(flab, eq_vram, (Vector2){roundf(g0 + 48), roundf(legend_y)},
        (float)flab.baseSize, 0, ca(PUFF_GREEN, alpha));
    ...
    for (int i = 0; i < n_sps_ticks; i++) {
        ...
        Font ft = tfont_fit(body, (int)(ts + 0.5f));
        Vector2 sz = MeasureTextEx(ft, tick, (float)ft.baseSize, 0);
        DrawTextEx(ft, tick, (Vector2){roundf(LEFT - 14.0f - sz.x), roundf(y - sz.y * 0.5f)},
            (float)ft.baseSize, 0, ca(PUFF_CYAN, alpha));
    }
    for (int i = 0; i < n_vram_ticks; i++) {
        ...
        Font ft = tfont_fit(body, (int)(ts + 0.5f));
        Vector2 sz = MeasureTextEx(ft, tick, (float)ft.baseSize, 0);
        DrawTextEx(ft, tick, (Vector2){roundf(RIGHT + 14.0f), roundf(y - sz.y * 0.5f)},
            (float)ft.baseSize, 0, ca(PUFF_GREEN, alpha));
    }
    for (int i = 0; i < N_PTS; i++) {
        float x = map_x(HIDDEN[i]);
    ...
    Font fa = tfont_fit(body, (int)(axis + 0.5f));
    DrawTextPro(fa, "SPS", (Vector2){box.x + 18.0f, (TOP + BOTTOM) * 0.5f},
        (Vector2){MeasureTextEx(fa, "SPS", (float)fa.baseSize, 0).x * 0.5f, 10.0f},
        -90.0f, (float)fa.baseSize, 0, ca(PUFF_CYAN, alpha));
    DrawTextPro(fa, "VRAM (GB)", (Vector2){box.x + box.width - 16.0f, (TOP + BOTTOM) * 0.5f},
        (Vector2){MeasureTextEx(fa, "VRAM (GB)", (float)fa.baseSize, 0).x * 0.5f, 10.0f},
        90.0f, (float)fa.baseSize, 0, ca(PUFF_GREEN, alpha));
