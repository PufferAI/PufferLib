...
// Run:    ./resources/trailer/trailer
// Record: ./resources/trailer/trailer --record resources/trailer/shots/puffer5.mp4
// Clips:  resources/trailer/clips/<name>.mp4 (ffmpeg pipe; raylib has no video API)
//
...
static const NewResult NEWS[N_NEW] = {
    ...
    {"robocode",      "Robocode",       NULL, NULL},
    {"craftax",       "Craftax",        NULL, NULL},
    {"nethack",       "NetHack",        NULL, NULL},
    {"zukbest_clip",  "OSRS Inferno & Colosseum", "colobest_clip", NULL},
};
...
static void text_c(Font f, const char *s, float cx, float y, float sz, Color c) {
    float w = tfont_width(f, s, sz, 0.4f);
    tfont_draw(f, s, cx - w * 0.5f, y, sz, 0.4f, c);
}
...
static void ensure_shots_dir(void) {
    ...
    mkdir("resources/trailer", 0755);
    mkdir("resources/trailer/shots", 0755);
}
...
// Each clip cell is [title above][16:9 video]; extra vertical margin absorbs the title strip.
static void results_layout(Rectangle cells[6]) {
    const float gap = 36.0f, mx = 48.0f;
    const float title_h = 42.0f;
    float w = (SCREEN_W - 2.0f * mx - 2.0f * gap) / 3.0f;
    float vid_h = w * 9.0f / 16.0f;
    float h = title_h + vid_h;
...
static float probe_fps_frac(const char *s) {
    int num = 0, den = 1;
    if (!s || sscanf(s, "%d/%d", &num, &den) < 1 || num <= 0) return 0;
    if (den <= 0) den = 1;
...
static int probe_video(const char *path, int *w, int *h, float *fps, float *dur) {
    ...
    snprintf(cmd, sizeof cmd,
        ...
        "-show_entries format=duration -of default=nw=1:nk=1 %s",
        path);
    FILE *fp = popen(cmd, "r");
    if (!fp) return 0;
    char line[128];
    char avg_s[64] = {0}, r_s[64] = {0};
    int nb = 0;
    *w = 0;
    *h = 0;
    *dur = 0;
    ...
    while (fgets(line, sizeof line, fp)) {
        ...
        case 2: snprintf(avg_s, sizeof avg_s, "%s", line); break;
        case 3: snprintf(r_s, sizeof r_s, "%s", line); break;
        case 4: nb = atoi(line); break;
        case 5: *dur = (float)atof(line); break;
        }
    }
    pclose(fp);
    float avg = probe_fps_frac(avg_s);
    float r = probe_fps_frac(r_s);
    float from_dur = (nb > 1 && *dur > 0.05f) ? (float)nb / *dur : 0;
    *fps = avg;
    if (*fps < 1.0f || *fps > 120.0f) *fps = r;
    if (*fps < 1.0f || *fps > 120.0f) *fps = from_dur;
...
static void load_clip(GameClip *c, const char *key) {
    memset(c, 0, sizeof *c);
    snprintf(c->path, sizeof c->path, "resources/trailer/clips/%s.mp4", key);
...
static void draw_results(Trailer *tr, float local, float alpha) {
    ...
    for (int i = 0; i < N_NEW; i++) {
        Rectangle cell = cells[i + 1];
        const float title_h = 42.0f;
        text_c(tr->ui, NEWS[i].name, cell.x + cell.width * 0.5f, cell.y + 4, 26,
               calpha(C_WHITE, fa));
        Rectangle card = {cell.x, cell.y + title_h, cell.width, cell.height - title_h};
        ...
        float split = 5.0f;
        int use2 = NEWS[i].key2 && local >= split;
        if (NEWS[i].key2) {
            GameClip *idle = use2 ? &tr->clip[i] : &tr->clip2[i];
            if (idle->pid > 0) clip_close(idle);
        }
        GameClip *gc = use2 ? &tr->clip2[i] : &tr->clip[i];
        float clip_t = use2 ? local - split : local;
        if (gc->tex.id) {
            Rectangle inner = {card.x + 10, card.y + 10, card.width - 20, card.height - 20};
            draw_clip(gc, clip_t, inner, fa);
        }
    }
}
...
static void draw_end(Trailer *tr, const Beat *b, float local, float alpha) {
    ...
    if (tr->puffer.tex.id) {
        ...
        // Banner is 960x480, fish on the left. Inflated face/spikes go to
        // ~x=484; the 2D logo stream is to the right of that.
        float src_w = tr->puffer.w > 486 ? 486.0f : (float)tr->puffer.w;
    ...
    float w1 = tfont_width(tr->ui, left, FONT_SUB, 0.4f);
    float w2 = tfont_width(tr->ui, right, FONT_SUB, 0.4f);
    float x = SCREEN_W * 0.5f - 0.5f * (w1 + w2);
    float y = 860.0f + 72.0f;
    tfont_draw(tr->ui, left, x, y, FONT_SUB, 0.4f, calpha(C_WHITE, alpha));
    tfont_draw(tr->ui, right, x + w1, y, FONT_SUB, 0.4f, calpha(C_CYAN, alpha));
}
...
static void draw_hud(Trailer *tr) {
    ...
    snprintf(buf, sizeof buf, "%.1fs / %.1fs    space pause   n next   r restart",
             tr->t, tr->total);
    tfont_draw(tr->mono, buf, 40, SCREEN_H - 36, 16, 0.4f, calpha(C_MUTED, 0.55f));
}
...
static void usage(void) {
    fprintf(stderr,
        "usage: ./resources/trailer/trailer [flags]\n"
        "  --shot [FILE]     export a still (default resources/trailer/shots/puffer5.png)\n"
        "  --plot [FILE]     full-screen scale plot still\n"
        "  --record [FILE]   record one pass (default resources/trailer/shots/puffer5.mp4)\n"
...
int main(int argc, char **argv) {
    ...
    const char *record_path = "resources/trailer/shots/puffer5.mp4";
    float seek = -1.0f;

    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--shot") == 0) {
            shot = "resources/trailer/shots/puffer5.png";
            if (i + 1 < argc && argv[i + 1][0] != '-') shot = argv[++i];
        } else if (strcmp(argv[i], "--plot") == 0) {
            plot_only = 1;
            shot = "resources/trailer/shots/fig_scale.png";
            if (i + 1 < argc && argv[i + 1][0] != '-') shot = argv[++i];
    ...
    for (int e = 0; e < N_NEW; e++) {
        ...
        if (NEWS[e].key2)
            load_clip(&tr.clip2[e], NEWS[e].key2);
    }
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_S)) {
            ensure_shots_dir();
            draw_trailer(&tr);
            export_png("resources/trailer/shots/puffer5.png");
        }
