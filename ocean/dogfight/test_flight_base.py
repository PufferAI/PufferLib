...
TEST_HIGHLIGHTS = {
    ...
    # High-speed oscillation tests
    'high_speed_pitch_oscillation': [3, 4, 5],  # speed, pitch, roll - watch for oscillations
    'high_speed_roll_oscillation': [3, 4, 5],   # speed, pitch, roll - watch for oscillations
    'speed_sweep_stability': [3, 4],            # speed, pitch - compare across speeds
}
...
def level_flight_pitch_from_state(env, kp=LEVEL_FLIGHT_KP, kd=LEVEL_FLIGHT_KD):
    ...
    NOTE: This outputs POSITION commands. For delta control, use
    level_flight_pitch_velocity() instead.
    """
...
def level_flight_pitch_velocity(env, kp=LEVEL_FLIGHT_KP, kd=LEVEL_FLIGHT_KD,
                                 coeff=DEFAULT_CONTROL_RATE_COEFF):
    """
    Velocity-based PD autopilot for level flight (vz = 0).
    Works with delta action space where actions are velocity commands.

    For delta control:
      ctrl_elevator += coeff * action[1]  (each tick)

    This function:
    1. Computes target elevator position (same as level_flight_pitch_from_state)
    2. Reads current ctrl_elevator from state
    3. Outputs velocity to drive toward target

    Args:
        env: Dogfight environment
        kp: Proportional gain on vz error
    ...
    # Target elevator position (same logic as position-based)
    target_elevator = np.clip(-kp * vz - kd * vz, -0.2, 0.2)
