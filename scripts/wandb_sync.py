...
def main():
    ...
    ap.add_argument("--env", default=None, help="env name for env-compiled binaries")
    ...
    if args.cmd:
        ...
        if os.path.exists(path):
            os.remove(path)
        child = subprocess.Popen(args.cmd + ["--base.wandb=1", f"--base.run_id={name}"])
    else:
