...
def setup(env_creator, env_args, env_kwargs):
    # breakpoint()
    # print(f'env_args VECTOR52={env_args} || env_kwargs={env_kwargs}')
    
    if isinstance(env_kwargs, argparse.Namespace):
        env_kwargs = vars(env_kwargs)  # Convert Namespace to dict
    else:
        env_kwargs = env_kwargs  # Use as is if it's already a dict
