...
void init_drone(Drone* drone, float size, float dr) {
    ...
    Params* params = &drone->params;

    params->arm_len = size / 2.0f;

    // m ~ x^3
    float mass_scale = powf(params->arm_len, 3.0f) / powf(BASE_ARM_LEN, 3.0f);
    params->mass = BASE_MASS * mass_scale * rndf(1.0f - dr, 1.0f + dr);

    // I ~ mx^2
    float base_Iscale = BASE_MASS * BASE_ARM_LEN * BASE_ARM_LEN;
    float I_scale = params->mass * powf(params->arm_len, 2.0f) / base_Iscale;
    params->ixx = BASE_IXX * I_scale * rndf(1.0f - dr, 1.0f + dr);
    params->iyy = BASE_IYY * I_scale * rndf(1.0f - dr, 1.0f + dr);
    params->izz = BASE_IZZ * I_scale * rndf(1.0f - dr, 1.0f + dr);

    // k_thrust ~ m/l
    float k_thrust_scale = (params->mass * params->arm_len) / (BASE_MASS * BASE_ARM_LEN);
    params->k_thrust = BASE_K_THRUST * k_thrust_scale * rndf(1.0f - dr, 1.0f + dr);

    // k_ang_damp ~ I
    float base_avg_inertia = (BASE_IXX + BASE_IYY + BASE_IZZ) / 3.0f;
    float avg_inertia = (params->ixx + params->iyy + params->izz) / 3.0f;
    float avg_inertia_scale = avg_inertia / base_avg_inertia;
    params->k_ang_damp = BASE_K_ANG_DAMP * avg_inertia_scale * rndf(1.0f - dr, 1.0f + dr);

    // drag ~ x^2
    float drag_scale = powf(params->arm_len, 2.0f) / powf(BASE_ARM_LEN, 2.0f);
    params->k_drag = BASE_K_DRAG * drag_scale * rndf(1.0f - dr, 1.0f + dr);
    params->b_drag = BASE_B_DRAG * drag_scale * rndf(1.0f - dr, 1.0f + dr);

    // Small gravity randomization
    params->gravity = BASE_GRAVITY * rndf(0.99f, 1.01f);

    // RPM ~ 1/x
    float rpm_scale = (BASE_ARM_LEN) / (params->arm_len);
    params->max_rpm = BASE_MAX_RPM * rpm_scale * rndf(1.0f - dr, 1.0f + dr);

    params->max_vel = BASE_MAX_VEL;
    params->max_omega = BASE_MAX_OMEGA;

    for (int i = 0; i < 4; i++) {
        state->rpms[i] = 0.0f;
    }
    params->k_mot = BASE_K_MOT * rndf(1.0f - dr, 1.0f + dr);
    params->j_mot = BASE_J_MOT * I_scale * rndf(1.0f - dr, 1.0f + dr);
}

void compute_derivatives(Params* params, State* state, float* actions, StateDerivative* derivatives) {
    // first order rpm lag
    float target_rpms[4];
    for (int i = 0; i < 4; i++) {
...
static void evaluate_step(const State* initial, const StateDerivative* deriv, float dt, State* output) {
...
void rk4(Drone* drone, float* actions, float dt) {
    StateDerivative k1, k2, k3, k4;
    State temp_state;

    compute_derivatives(&drone->params, &drone->state, actions, &k1);

    evaluate_step(&drone->state, &k1, dt * 0.5f, &temp_state);
    compute_derivatives(&drone->params, &temp_state, actions, &k2);

    evaluate_step(&drone->state, &k2, dt * 0.5f, &temp_state);
    compute_derivatives(&drone->params, &temp_state, actions, &k3);

    evaluate_step(&drone->state, &k3, dt, &temp_state);
    compute_derivatives(&drone->params, &temp_state, actions, &k4);

    float dt_6 = dt / 6.0f;

    drone->state.pos.x += (k1.vel.x + 2.0f * k2.vel.x + 2.0f * k3.vel.x + k4.vel.x) * dt_6;
    drone->state.pos.y += (k1.vel.y + 2.0f * k2.vel.y + 2.0f * k3.vel.y + k4.vel.y) * dt_6;
    drone->state.pos.z += (k1.vel.z + 2.0f * k2.vel.z + 2.0f * k3.vel.z + k4.vel.z) * dt_6;

    drone->state.vel.x += (k1.v_dot.x + 2.0f * k2.v_dot.x + 2.0f * k3.v_dot.x + k4.v_dot.x) * dt_6;
    drone->state.vel.y += (k1.v_dot.y + 2.0f * k2.v_dot.y + 2.0f * k3.v_dot.y + k4.v_dot.y) * dt_6;
    drone->state.vel.z += (k1.v_dot.z + 2.0f * k2.v_dot.z + 2.0f * k3.v_dot.z + k4.v_dot.z) * dt_6;

    drone->state.quat.w += (k1.q_dot.w + 2.0f * k2.q_dot.w + 2.0f * k3.q_dot.w + k4.q_dot.w) * dt_6;
    drone->state.quat.x += (k1.q_dot.x + 2.0f * k2.q_dot.x + 2.0f * k3.q_dot.x + k4.q_dot.x) * dt_6;
    drone->state.quat.y += (k1.q_dot.y + 2.0f * k2.q_dot.y + 2.0f * k3.q_dot.y + k4.q_dot.y) * dt_6;
    drone->state.quat.z += (k1.q_dot.z + 2.0f * k2.q_dot.z + 2.0f * k3.q_dot.z + k4.q_dot.z) * dt_6;

    drone->state.omega.x += (k1.w_dot.x + 2.0f * k2.w_dot.x + 2.0f * k3.w_dot.x + k4.w_dot.x) * dt_6;
    drone->state.omega.y += (k1.w_dot.y + 2.0f * k2.w_dot.y + 2.0f * k3.w_dot.y + k4.w_dot.y) * dt_6;
    drone->state.omega.z += (k1.w_dot.z + 2.0f * k2.w_dot.z + 2.0f * k3.w_dot.z + k4.w_dot.z) * dt_6;

    for (int i = 0; i < 4; i++) {
        drone->state.rpms[i] += (k1.rpm_dot[i] + 2.0f * k2.rpm_dot[i] + 2.0f * k3.rpm_dot[i] + k4.rpm_dot[i]) * dt_6;
    }

    quat_normalize(&drone->state.quat);
}
