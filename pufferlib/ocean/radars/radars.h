...
void allocate(Radars *env) {
  env->observations =
      (int16_t *)calloc((MAX_AZ_SLICES * MAX_EL_SLICES + env->max_trackers * FEATURES_PER_TRACKER +
...
void c_step(Radars *env) {
  ...
  if (action == SEARCH) {
    if (env->s_band_t_until_free == 0) {
      ...
      env->observations[least_recently_used_cluster] = ZERO_COST_SEARCH_TIME;

      search_sector(env, (least_recently_used_cluster + 1) % (MAX_AZ_SLICES * MAX_EL_SLICES));
      env->observations[(least_recently_used_cluster + 1) % (MAX_AZ_SLICES * MAX_EL_SLICES)] =
          ZERO_COST_SEARCH_TIME;

      search_sector(
          env, (least_recently_used_cluster + MAX_AZ_SLICES) % (MAX_AZ_SLICES * MAX_EL_SLICES));
      env->observations[(least_recently_used_cluster + MAX_AZ_SLICES) %
                        (MAX_AZ_SLICES * MAX_EL_SLICES)] = ZERO_COST_SEARCH_TIME;

      search_sector(
          env, (least_recently_used_cluster + MAX_AZ_SLICES + 1) % (MAX_AZ_SLICES * MAX_EL_SLICES));
      env->observations[(least_recently_used_cluster + MAX_AZ_SLICES + 1) %
                        (MAX_AZ_SLICES * MAX_EL_SLICES)] = ZERO_COST_SEARCH_TIME;
  ...
  if (delta_t > 0) {
    ...
    for (int i = 0; i < env->max_trackers; i++) {
      if (env->targets[i].x < 0 || env->targets[i].x > MAX_TARGET_XY_RANGE ||
          ...
          env->targets[i].x * env->targets[i].x + env->targets[i].y * env->targets[i].y +
                  env->targets[i].z * env->targets[i].z >
...
void c_render(Client *client, Radars *env) {
  ...
  int action = env->actions[0];
  if (action == SEARCH) {
    DrawText("SEARCH", 150 * client->scale, 5 * client->scale, 10 * client->scale, WHITE);
  } else if (action <= env->max_trackers) {
    DrawText(TextFormat("TRACK %d", action), 150 * client->scale, 5 * client->scale,
             10 * client->scale, WHITE);
    action -= 1;
    if (env->targets[action].is_tracked) {
      DrawCircle(env->targets[action].x * ppmm,
                 WINDOW_Y_PX * client->scale - env->targets[action].y * ppmm, 10 * client->scale,
                 COLORS[6]);
    }
  }
