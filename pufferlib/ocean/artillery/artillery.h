...
float calculate_parabola_closest_distance(Artillery* env, int gun_idx) {
...
void fire_projectile(Artillery* env, int gun_idx) {
    Gun* gun = &env->gun[gun_idx];
...
void step_frame(Artillery* env, float action0, float action1) {
...
Client* make_client(Artillery* env) {
    ...
    InitWindow(WIDTH, HEIGHT, "PufferLib Artillery Dual");
    SetTargetFPS(30);
