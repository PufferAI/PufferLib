/* Overcooked: a multiagent cooking coordination environment.
 ...
 */
void compute_observations(Overcooked* env) {
    int obs_idx = 0;
    for (int a=0; a<env->num_agents; a++) {
        Agent* agent = &env->agents[a];
        for (int g=0; g<env->num_goals; g++) {
...
void c_render(Overcooked* env) {
    if (env->client == NULL) {
        InitWindow(env->width, env->height, "PufferLib Overcooked");
        SetTargetFPS(60);
