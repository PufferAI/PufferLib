...
void c_render(Dogfight *env) {
    // Phase 6: Raylib rendering
    if (env->client == NULL) {
        env->client = (Client *)calloc(1, sizeof(Client));
        env->client->width = 800;
        env->client->height = 600;
        InitWindow(800, 600, "Dogfight");
        SetTargetFPS(60);

        env->client->camera.position = (Vector3){10.0f, 10.0f, 10.0f};
        env->client->camera.target = (Vector3){0.0f, 0.0f, 0.0f};
        env->client->camera.up = (Vector3){0.0f, 1.0f, 0.0f};
    ...
    DrawGrid(10, 1.0f);
    EndMode3D();
    DrawText("Dogfight - Phase 0", 10, 10, 20, WHITE);
    DrawText(TextFormat("Tick: %d", env->tick), 10, 40, 20, WHITE);
    EndDrawing();
}
