...
def local_to_global_coord(
    ...
    min_x = np.min([e[0] for e in map_offsets.values()])
    min_y = np.min([e[1] for e in map_offsets.values()])
    if map_idx in map_offsets.keys():
        dx, dy = map_offsets[map_idx]
        x, y = x - min_x, y - min_y
    else:
        offset = np.array([0,0])
        x, y = 0, 0
...
def save_counts_map_as_hue(counts_map, filename):
    # Scale the counts for hue value from 0 to 1 (corresponds to 0-255)
    max_count = np.max(counts_map)
    hue_scale = 255 if max_count == 0 else 255 / max_count

    # Initialize HSV image: hue from counts, saturation and value to be determined
    hsv_image = np.zeros((*counts_map.shape, 3), dtype=float)

    # Set hue based on scaled counts, saturation and value as described
    hsv_image[..., 0] = counts_map * hue_scale / 255.0  # Hue scaled to [0, 1]
    hsv_image[..., 1] = np.where(counts_map > 0, 1, 0)   # Saturation
    hsv_image[..., 2] = np.where(counts_map > 0, 1, 0)   # Value
...
class PokemonRed(Env):
    def __init__(
        ...
        if self.extra_buttons:
            self.valid_actions.extend([
                WindowEven1024t.PRESS_BUTTON_START,
                WindowEvent.PASS
            ])
    ...
    def step(self, action):
        ...
        if step_limit_reached:
            path = self.s_path / Path(f'counts_{self.reset_count}_{self.instance_id}.png')
            np.save(path, self.counts_map)
